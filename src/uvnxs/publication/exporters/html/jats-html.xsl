...
<!--  <xsl:param name="css" select="'jats-preview.css'"/>-->
  ...
  <xsl:template name="make-html-header">
    <head>
      ...
      <style>
        .metadata-label { color: #6c757d; font-size: .8rem; font-weight: 600; min-width: 9rem; display: inline-block; }
        .metadata-value { font-size: .875rem; }
        p.metadata-entry { margin-bottom: .25rem; }
        .metadata-group { padding-left: .25rem; }
      </style>
...
<!--    <hr class="part-rule"/>-->
      ...
      <xsl:for-each select="article-meta | self::front-stub">
        ...
        <xsl:if test="abstract[normalize-space(string(*[not(self::title)]))] | trans-abstract[normalize-space(string(*[not(self::title)]))]">
          ...
          <xsl:for-each select="abstract[normalize-space(string(*[not(self::title)]))] | trans-abstract[normalize-space(string(*[not(self::title)]))]">
...
<!--  Includes mode "metadata" for front matter, along with
  ...
  <xsl:template match="fig-count"
    mode="metadata-label">Abbildungen</xsl:template>


  <xsl:template match="table-count"
    mode="metadata-label">Tabellen</xsl:template>


  <xsl:template match="equation-count"
    mode="metadata-label">Gleichungen</xsl:template>


  <xsl:template match="ref-count"
    mode="metadata-label">Referenzen</xsl:template>


  <xsl:template match="page-count"
    mode="metadata-label">Seiten</xsl:template>


  <xsl:template match="word-count"
    mode="metadata-label">Wörter</xsl:template>
  ...
  <xsl:template match="table">
    <xsl:copy>
      ...
      <xsl:attribute name="class">
        <xsl:choose>
          <xsl:when test="@class">table table-bordered <xsl:value-of select="@class"/></xsl:when>
  ...
  <xsl:template name="metadata-labeled-entry">
    ...
    <xsl:call-template name="metadata-entry">
      <xsl:with-param name="contents">
        ...
        <span class="metadata-value"><xsl:copy-of select="$contents"/></span>
      </xsl:with-param>
    </xsl:call-template>
  </xsl:template>
